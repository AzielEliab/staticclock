...
async function timeslateOf(tick) {
  const payload = {
    click: tick.click,
...
function openapiSpec() {
  ...
  return {
    ...
    info: {
      ...
      version: VERSION,
      description: "Action-based immutable timeline. No rollbacks. AZ-OS hook. " + MOTTO + " Suite mesh /v1/mesh/* PROXY to aziel-runtime (AZIEL_RUNTIME). Default OFF. QNM-BUILD-1.0 live|locked|isolated. QNS-CD-1.0 photon QNS1 packet transfer cross-map (no public qnsd proxy). No Node Gate. No auto-heal. Not anonymity. Not a Softwares-tab product. StaticClock is Plain category (not Lock). Aziel Eliab only.",
    },
    servers: [{ url: HOST }],
    paths: {
      "/v1/skill": {
...
<title>StaticClock — use with AI assistants</title>
<style>
...
<body>
  ...
  <p>Paste this OpenAPI URL into ChatGPT GPT Actions, Claude connectors, Microsoft Copilot / Bing, Google Gemini / Vertex, Grok custom tools, Venice HTTP tools, Perplexity, Mistral, Meta AI, Amazon Q, Cohere, DuckAssist, You.com, and other OpenAPI-capable assistants:</p>
  ...
  <p>Suite mesh: <code>GET ${HOST}/v1/mesh</code> PROXY to aziel-runtime. Default OFF. QNM-BUILD-1.0 live|locked|isolated. QNS-CD-1.0 photon QNS1 packet transfer is a hub cite / Worker mesh cross-map only (qnm-node local qnsd; aziel-runtime cites; AZInterface pair custody). No Node Gate. No public qnsd proxy. No auto-heal. Not anonymity. Not a Softwares-tab product. StaticClock is Plain category (not Lock). Author: Aziel Eliab only.</p>
...
export async function handleRuntimeApi(request, url) {
  ...
  if (path === "/v1/timeslate" && request.method === "POST") {
    let body;
    try { body = await request.json(); } catch { return json({ error: "JSON body required" }, 400); }
    const clicks = asClicks(body && body.clicks);
    if (!clicks.length) return json({ empty: true, timeslate: null, product: PRODUCT, author: AUTHOR });
    return json({ product: PRODUCT, version: VERSION, author: AUTHOR, ...(await timeslateOf(clicks[clicks.length - 1])) });
  }
