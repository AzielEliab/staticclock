...
PRODUCT_JS = (ROOT / "workers/download-tracker/public/product.js").read_text(encoding="utf-8")
...
def test_mesh_contract_default_off_qnm_law() -> None:
    ...
    assert 'MESH_PRODUCT = "staticclock"' in MESH
    assert 'MESH_CATEGORY = "Plain"' in MESH
    assert "not Lock" in MESH
...
def test_mesh_proxies_via_aziel_runtime() -> None:
    ...
    assert 'id = "3846ce9f571043afbe67e5fff90a0181"' in WRANGLER
...
def test_home_live_nodes_strip_no_node_gate() -> None:
    ...
    assert "Plain category (not Lock)" in HOMEPAGE
    assert 'product: "staticclock"' in PRODUCT_JS
